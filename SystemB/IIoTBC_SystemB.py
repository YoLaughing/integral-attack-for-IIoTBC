    ...
    def round_Function(self):
        ...
        for rnd in range(self.round_num):
            ...
            for g in range(self.fi_num):
                # copy x --> (y,z)
                begin = g *32
                MILP_eqn += self.constraint_copy(in_vars[begin:begin + 8]+in_vars[begin+16:begin + 24],
                                                 yi[begin:begin + 8]+yi[begin+16:begin + 24],
                                                 zi[begin:begin + 8]+zi[begin+16:begin + 24])
                # four sbox
                for s_i in range(4):
                    s_iin = [zi[int(p)+begin] for p in self.sbox_p[s_i]]
                    s_iout = [zi[int(p)+begin+8] for p in self.sbox_p[s_i]]
                    MILP_eqn += self.sbox(s_iin, s_iout)
                ...
                begin += 16
                zi_ll1.append(zi[begin+7])
                zi_ll1 += zi[begin:begin+7]
                begin = 8 + g * 32
                MILP_eqn += self.constraint_xor2(in_vars[begin:begin + 8]+in_vars[begin+16:begin + 24], zi_ll1, yi[begin:begin + 8]+yi[begin+16:begin + 24])
